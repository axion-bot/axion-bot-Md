...
export default async function (m, conn = { user: {} }) {
  ...
  try {
    ...
    let realType = m.mtype
    if ((!realType || realType === 'unknown') && m.message) {
      const keys = Object.keys(m.message)
      realType = keys.find(v => v !== 'messageContextInfo' && v !== 'senderKeyDistributionMessage') || keys[0]
    }
    const tipo = (realType || 'unknown').replace(/Message/gi, '').toUpperCase()
