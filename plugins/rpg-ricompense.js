...
let bonus=Math.floor(Math.random()*51),evento='*✨ 𝐁𝐨𝐧𝐮𝐬 𝐧𝐨𝐫𝐦𝐚𝐥𝐞*'
const roll=Math.random()
if(roll<0.01){bonus+=1000;evento='*💎 𝐉𝐀𝐂𝐊𝐏𝐎𝐓 𝐌𝐄𝐒𝐒𝐀𝐆𝐆𝐈*'}
else if(roll<0.04){bonus+=300;evento='*🔥 𝐒𝐔𝐏𝐄𝐑 𝐁𝐎𝐍𝐔𝐒*'}
else if(roll<0.19){bonus+=100;evento='*⚡ 𝐁𝐎𝐍𝐔𝐒 𝐑𝐀𝐑𝐎*'}
return{bonus,evento}
}
...
const premiText=premiati.length?premiati.slice(0,10).map(v=>`• @${v.jid.split('@')[0]} — *${formatNumber(v.count)} msg* *(+${formatNumber(v.premio)}€)*`).join('\n'):'*𝐍𝐞𝐬𝐬𝐮𝐧 𝐩𝐫𝐞𝐦𝐢𝐨 𝐚𝐬𝐬𝐞𝐠𝐧𝐚𝐭𝐨 𝐝𝐢 𝐫𝐞𝐜𝐞𝐧𝐭𝐞.*'
await conn.sendMessage(m.chat,{text:
...
user.euro+=reward
user.lastDaily=now
return m.reply(`🎉 *𝐑𝐢𝐜𝐨𝐦𝐩𝐞𝐧𝐬𝐚 𝐆𝐢𝐨𝐫𝐧𝐚𝐥𝐢𝐞𝐫𝐚 𝐑𝐢𝐭𝐢𝐫𝐚𝐭𝐚! 🥳*
