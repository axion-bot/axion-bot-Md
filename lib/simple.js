...
const mtype = getContentType(normalizedMessage) || ''
            ...
            if (normalizedMessage?.[mtype]?.contextInfo?.quotedMessage) {
              /**
               * @type {import('@realvare/baileys').proto.IContextInfo}
               */
              const context = normalizedMessage[mtype].contextInfo;
               ...
               *
               */
              const quoted = normalizedMessage[mtype].contextInfo.quotedMessage;
              if ((remoteJid && remoteJid !== 'status@broadcast') && quoted) {
                let  qMtype = Object.keys (
quoted)[0]; 
...
mtype: {
  get() {
    ...
    const normalized = normalizeMessageContent(this.message)
    return getContentType(normalized) || ''
  },
  enumerable: true,
},
msg: {
  get() {
    ...
    return type ? normalized[type] : null
  },
},
    mediaMessage: {
      get() {
