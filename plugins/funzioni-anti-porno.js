...
handler.before=async function(m,{conn,isAdmin,isOwner,isROwner,isBotAdmin}){
  ...
  if(txt.includes('http')&&nsfwKeywords.some(keyword=>txt.includes(keyword))){
    return await punishUser(conn,m,isBotAdmin,'𝐋𝐢𝐧𝐤 𝐍𝐒𝐅𝐖 𝐫𝐢𝐥𝐞𝐯𝐚𝐭𝐨')
  }
...
async function punishUser(conn,m,isBotAdmin,reason){
  const data=global.addGroupWarn(m.sender,m.chat,'antiporno','system')
